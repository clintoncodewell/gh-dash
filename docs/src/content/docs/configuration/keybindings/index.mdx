...
Add `successMessage` when you want a short confirmation in the footer after a
...
keybindings:
  issues:
    - key: H
      name: launch agent
      footer: agent
      command: launch-agent {{.RepoName}} {{.IssueNumber}}
      successMessage: Agent launched
```

Set `footer: agent` on one PR or issue command to expose it as the clickable
agent control in the footer.
